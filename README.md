...
The Lambda function code is in src/index.py.
The CloudFormation template is in infrastructure/template.yaml.
The deployment script is in deployment/deploy.sh.
...
The data synchronization part of the Lambda function which fetches data from both the BLS API and the population data API is triggered by the scheduled event, we need to simulate that event for test.
...
Test the function
aws lambda invoke \
    --function-name data-pipeline-pipeline \
